...
## F-20 — R2 built: pitch accuracy established, audio quality deliberately not claimed (2026-07-30, DT-100)

`src/dsp_engine/psola.py`, `tests/test_psola.py`.

F-17 blocked DT-100 on R2, a real time-varying resynthesis stage, because approximating a correction curve by slicing and concatenating fixed shifts measured SI-SDR −23.7..−33.7 dB against a single-call shift. R2 is now built as TD-PSOLA: grains centred on pitch marks, overlap-added at re-spaced positions, so nothing is cut mid-period and there is no seam to hear.

**What is established.** Requested pitch is reached, verified with the independent R1 estimator rather than by trusting the transform: within **±0.4 cents** across −900..+1200 cents on synthetic tones, and **+0.29 / −0.28 / +0.09 cents** at +50 cents on the three Tier A vocal fixtures. Duration is preserved exactly, a per-sample curve is followed, and cost is **0.03–0.05× realtime**. Because grains are re-spaced rather than resampled, the spectral envelope stays put: measured envelope movement at +700 cents is **5.45 / 8.14 / 2.78 dB** against `PitchShift`'s **12.62 / 14.35 / 4.30 dB** — better on all three, which is the property that justifies R2 existing alongside the primitive.

**What is NOT established, and why the obvious measurement was rejected.** Unity-ratio SI-SDR against the input is poor and highly variable (−5.65 / −14.32 / **−39.84 dB**). That number is *not* being reported as a quality verdict, because SI-SDR is phase-sensitive and PSOLA reconstructs from grains whose placement can differ by up to half a period without any audible consequence. Comparing R2 against `PitchShift` by SI-SDR is worse still: it measures disagreement between two algorithms that *must* disagree, since one preserves formants and the other does not. Reporting either as "R2 beats/loses to the baseline" would repeat N-018 exactly — trusting a metric before establishing what it measures. **R2's audio quality is therefore unmeasured, and is recorded as unmeasured.** A phase-insensitive quality measure, or listening, is what would settle it.

**Failure mode caught during the build.** Pitch marks were snapped to the peak of `|audio|`, which offers two candidates per period on any waveform with a strong negative lobe. Consecutive marks alternated between the positive and negative excursion — measured spacings of 160/241 samples against an expected 200 — making the output periodic at *twice* the period and dropping it an exact octave, at every requested ratio including 1.0. Snapping to the signed maximum locks every grain to the same phase. A uniform −1200 cent error looks like a plausible transposition on a plot; it was only visible because the output was measured against a known input pitch.
